...
and parser cmd_ =
  | def name:(Term.bound_name) colon tp:(Term.term) assign tm:(Term.term) -> S.Def {name; tp; tm}
  | axiom name:(Term.bound_name) colon tp:(Term.term) -> S.Axiom {name; tp}
  | quit -> S.Quit
  | import u:name -> S.Import {unit_path = u; modifier = Yuujinchou.Modifier.seq []}
